...
class SimpleShiftSchema(BaseModel):
    ...
    date: str = Field(..., description="Date of the work shift as a string.")
    earned: float = Field(
...
class SpecificShift(BaseModel):
    ...
    earned_cold: float | None = Field(
        None, description="How much will be the extra payment for working in the cold."
    )
    earned_overtime: float | None = Field(
        None, description="How much will be the extra payment for overtime hours."
    )
    ...
    award_amount: float | None = Field(
        None, description="How much the user earned in the form of a bonus."
    )
    count_operations: float | None = Field(
        None, description="How many operations the user performed per shift."
    )
...
class ManyAddShifts(BaseModel):
    ...
    hours: float = Field(..., description="How many hours should I put in per shift?")
    dates: list[str] = Field(..., description="A list of dates for recording shifts.")
