...
async def get_salary_for_day(day_id: str, db: AsyncIOMotorDatabase) -> dict | None:
    """
...
async def update_salary(day_id: str, data: dict, db: AsyncIOMotorDatabase) -> None:
    """
    ...
    try:
        object_id = ObjectId(day_id)
        await collection.update_one({"_id": object_id}, {"$set": data}, upsert=True)
